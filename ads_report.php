...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold"><i class="bi bi-wallet2 text-primary"></i> Buku Besar Ads</h3>
    <button class="btn btn-warning fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#modalRate">
        <i class="bi bi-gear"></i> Set Persentase (%)
    </button>
</div>

<div class="row mb-4 g-3">
    <div class="col-md-7">
        <form method="GET" class="card shadow-sm border-0 h-100">
            <div class="card-body p-3 d-flex align-items-end gap-3">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-dark fw-bold px-3">Terapkan</button>
                    <a href="ads_report.php" class="btn btn-primary fw-bold px-3">Semua</a>
                </div>
            </div>
        </form>
    </div>
            ...
            <div class="col-6">
                <div class="card shadow-sm border-0 bg-success text-white h-100">
                    <div class="card-body p-3 text-center d-flex flex-column justify-content-center">
                        <span class="small text-uppercase fw-bold opacity-75">Omzet (<?php echo $label_filter; ?>)</span>
                        <h5 class="fw-bold mb-0 mt-1">Rp <?php echo number_format($total_omzet_real, 0, ',', '.'); ?></h5>
                    </div>
                </div>
            </div>
            <div class="col-6">
                <div class="card shadow-sm border-0 bg-primary text-white h-100">
                    <div class="card-body p-3 text-center d-flex flex-column justify-content-center">
                        <span class="small text-uppercase fw-bold opacity-75">Top-Up (<?php echo $label_filter; ?>)</span>
                        <h5 class="fw-bold mb-0 mt-1">Rp <?php echo number_format($tot_topup, 0, ',', '.'); ?></h5>
                    </div>
                </div>
            </div>
        </div>
        ...
        <table class="table table-hover align-middle mb-0 text-center">
            <thead class="table-light">
                <tr>
                    ...
                    <th class="text-center">Jatah / Rate</th>
            ...
            <tbody>
                ...
                <tr>
                    <td class="text-start ps-4 fw-bold text-secondary">
                        ...
                        <small class="fw-normal"><?php echo date('H:i', strtotime($h['tanggal_topup'])); ?></small>
                    </td>
                    <td><span class="badge bg-dark rounded-pill px-3"><?php echo $h['platform']; ?></span></td>
                    <td class="text-end">
                        <div class="fw-bold">Rp <?php echo number_format($h['sales_period_total'], 0, ',', '.'); ?></div>
                    ...
                    <td class="text-end fw-bold">
                        <?php if($h['remaining_balance'] < 0): ?>
                            ...
                            <div class="small fw-normal text-danger opacity-75">Hutang</div>
                        <?php else: ?>
                            <span class="text-primary">Rp <?php echo number_format($h['remaining_balance'], 0, ',', '.'); ?></span>
                            <div class="small fw-normal text-primary opacity-75">Sisa Saldo</div>
                        <?php endif; ?>
                    </td>
                    <td class="text-center pe-4">
                        <a href="process_ads.php?action=delete_topup&id=<?php echo $h['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Hapus riwayat top up ini? Semua perhitungan jatah di masa depan akan disesuaikan secara otomatis.')"><i class="bi bi-trash"></i></a>
                    </td>
                </tr>
                <?php endwhile; ?>
...
<div class="modal fade" id="modalRate" tabindex="-1">
    <div class="modal-dialog">
        <form action="process_ads.php?action=change_rate" method="POST" class="modal-content border-0">
            <div class="modal-header bg-warning">
                <h5 class="fw-bold mb-0">Update Jatah Ads</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4 text-center">
                <label class="fw-bold mb-2">Persentase Baru (%)</label>
                <input type="number" step="0.01" name="new_rate" class="form-control form-control-lg text-center fw-bold" value="<?php echo $current_rate; ?>" required>
            </div>
            <div class="modal-footer bg-light"><button type="submit" class="btn btn-dark w-100 py-2 fw-bold">Simpan Perubahan</button></div>
        </form>
    </div>
</div>
