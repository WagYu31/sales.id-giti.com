...
// Program Rule:
// Hitung seluruh penambahan Customer Baru, Follow Up, Invoice, dan Omset Penjualan di periode program secara lengkap
$sql_ranking_all = "
    SELECT 
        s.id AS sales_id,
...
.loewix-runner-3d, .nailong-sprite { display: none !important; }
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div class="d-flex align-items-center gap-2 mt-1.5 flex-wrap" style="font-size: 11px;">
                    <span class="badge bg-light text-secondary border px-2.5 py-1 rounded-pill d-inline-flex align-items-center gap-1.5 shadow-sm">
                        <span class="text-primary fw-bold">🎯 Syarat Program:</span> Customer Baru Input + Belanja (Ada Invoice) &bull; Customer Lama Reaktivasi (Terakhir Belanja &le; Bulan 5)
                    </span>
                </div>
            </div>
        </div>
            ...
            <div class="p-3.5 bg-light rounded-4 border h-100 d-flex flex-column justify-content-between shadow-sm position-relative">
                ...
                <div class="chart-scroll-wrapper" id="chartScrollWrapper">
                    ...
                    <div style="position: relative; width: 100%; min-height: 320px;" id="chartCanvasContainer">
                        ...
                        <!-- Real-time Three.js 3D WebGL Bipedal Kinematic Canvas -->
                        ...
                        <div id="cctvMascotOverlayHolder" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 20;"></div>
                    </div>
                </div>
            </div>
        </div>
...
/* ==========================================================================
   LOEWIX 3D WEBGL REAL-TIME KINEMATIC CHARACTER ENGINE (THREE.JS)
   - 3D Procedural Rigged Mecha Mesh with Hip, Knee, Shoulder, and Arm Kinematics
   - Real 3D Knee Flexion & Bipedal Stride Loop at 60 FPS
   ========================================================================== */
...
function initThreeJsRunnersEngine() {
    ...
    if (!isThreeJsEngineActive) {
        try {
            threeScene = new THREE.Scene();

            // Orthographic camera mapping screen pixel coordinates
            // left=0, right=width, top=height, bottom=0
            threeCamera = new THREE.OrthographicCamera(0, width, height, 0, 1, 1000);
        ...
        threeCamera.right = width;
        threeCamera.top = height;
...
function build3DLoewixMechaRobot() {
    ...
    const darkMetalMat = new THREE.MeshStandardMaterial({ color: 0x1E293B, metalness: 0.85, roughness: 0.25 });
    const lensMat = new THREE.MeshPhysicalMaterial({ color: 0x0F172A, metalness: 0.2, roughness: 0.05, transmission: 0.8, transparent: true });
    ...
    const domeHead = new THREE.Mesh(new THREE.SphereGeometry(12, 32, 32), chromeMat);
    headGroup.add(domeHead);

    const lensBezel = new THREE.Mesh(new THREE.CylinderGeometry(7, 7.5, 3.5, 32), darkMetalMat);
    lensBezel.rotation.x = Math.PI / 2;
    lensBezel.position.z = 9;
    headGroup.add(lensBezel);

    const lensGlass = new THREE.Mesh(new THREE.SphereGeometry(5.8, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2), lensMat);
    lensGlass.rotation.x = Math.PI / 2;
    lensGlass.position.z = 10;
    headGroup.add(lensGlass);

    const redLed = new THREE.Mesh(new THREE.SphereGeometry(1.4, 16, 16), redLedMat);
    redLed.position.set(3.5, 4.5, 11);
    headGroup.add(redLed);

    headGroup.position.y = 24;
    group.add(headGroup);

    // 2. PELVIS & TORSO
    const pelvis = new THREE.Mesh(new THREE.CylinderGeometry(5, 4.2, 6, 16), darkMetalMat);
    pelvis.position.y = 13;
    group.add(pelvis);

    // 3. LEFT LEG (Hip -> Thigh -> Knee -> Shin -> Foot)
    const leftHip = new THREE.Group();
    leftHip.position.set(-5, 13, 0);

    const leftThigh = new THREE.Mesh(new THREE.CylinderGeometry(2.2, 1.8, 10, 16), chromeMat);
    leftThigh.position.y = -5;
    leftHip.add(leftThigh);

    const leftKnee = new THREE.Group();
    leftKnee.position.y = -10;
    const leftKneeCap = new THREE.Mesh(new THREE.SphereGeometry(2, 16, 16), darkMetalMat);
    leftKnee.add(leftKneeCap);

    const leftShin = new THREE.Mesh(new THREE.CylinderGeometry(1.8, 1.4, 10, 16), chromeMat);
    leftShin.position.y = -5;
    leftKnee.add(leftShin);

    const leftFoot = new THREE.Mesh(new THREE.BoxGeometry(3.5, 2.2, 7), darkMetalMat);
    leftFoot.position.set(0, -10, 2);
    leftKnee.add(leftFoot);

    leftHip.add(leftKnee);
    group.add(leftHip);

    // 4. RIGHT LEG (Hip -> Thigh -> Knee -> Shin -> Foot)
    const rightHip = new THREE.Group();
    rightHip.position.set(5, 13, 0);

    const rightThigh = new THREE.Mesh(new THREE.CylinderGeometry(2.2, 1.8, 10, 16), chromeMat);
    rightThigh.position.y = -5;
    rightHip.add(rightThigh);

    const rightKnee = new THREE.Group();
    rightKnee.position.y = -10;
    const rightKneeCap = new THREE.Mesh(new THREE.SphereGeometry(2, 16, 16), darkMetalMat);
    rightKnee.add(rightKneeCap);

    const rightShin = new THREE.Mesh(new THREE.CylinderGeometry(1.8, 1.4, 10, 16), chromeMat);
    rightShin.position.y = -5;
    rightKnee.add(rightShin);

    const rightFoot = new THREE.Mesh(new THREE.BoxGeometry(3.5, 2.2, 7), darkMetalMat);
    rightFoot.position.set(0, -10, 2);
    ...
    leftShoulder.position.set(-11, 24, 0);
    const leftArm = new THREE.Mesh(new THREE.CylinderGeometry(1.8, 1.4, 11, 16), chromeMat);
    leftArm.position.y = -5.5;
    leftShoulder.add(leftArm);
    group.add(leftShoulder);

    const rightShoulder = new THREE.Group();
    rightShoulder.position.set(11, 24, 0);
    const rightArm = new THREE.Mesh(new THREE.CylinderGeometry(1.8, 1.4, 11, 16), chromeMat);
    rightArm.position.y = -5.5;
    rightShoulder.add(rightArm);

    // Flag Pole & Flag
    const flagPole = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.6, 24, 12), darkMetalMat);
    flagPole.position.set(0, -3, 6);
    flagPole.rotation.x = Math.PI / 4;
    rightShoulder.add(flagPole);

    const flagRed = new THREE.Mesh(new THREE.BoxGeometry(9, 3.5, 0.3), flagRedMat);
    flagRed.position.set(4.5, 6, 10);
    rightShoulder.add(flagRed);

    const flagWhite = new THREE.Mesh(new THREE.BoxGeometry(9, 3.5, 0.3), flagWhiteMat);
    flagWhite.position.set(4.5, 2.5, 10);
    rightShoulder.add(flagWhite);

    group.add(rightShoulder);

    // Scale & Rotation facing rightwards (towards finish line)
...
function animateThreeJsRunnersLoop() {
    ...
    threeRobotsList.forEach((robot, idx) => {
        ...
        // Bipedal Stride Sine-Wave Kinematics
        const strideAngle = Math.sin(time * 0.9 + idx * 0.4) * 0.85;

        // Hip Joint Stride
        robot.leftHip.rotation.x = strideAngle;
        robot.rightHip.rotation.x = -strideAngle;

        // Knee Joint Flexion (Real knee bending during steps)
        robot.leftKnee.rotation.x = Math.max(0, Math.sin(time * 0.9 + idx * 0.4 + 0.4) * 1.25);
        robot.rightKnee.rotation.x = Math.max(0, -Math.sin(time * 0.9 + idx * 0.4 + 0.4) * 1.25);

        // Shoulder & Arm Swing
        robot.leftShoulder.rotation.x = -strideAngle * 0.8;
        robot.rightShoulder.rotation.x = strideAngle * 0.8;

        // Sprint Body Lean & Bounce
        robot.group.rotation.z = -0.24; // Lean forward into sprint
        robot.group.position.x = robot.targetX;
        robot.group.position.y = robot.targetY + Math.abs(Math.sin(time * 0.9)) * 3.5;
    });
    ...
    meta.data.forEach((bar, index) => {
        ...
        if (has3D) {
            ...
            if (threeRobotsList[index]) {
                ...
                threeRobotsList[index].targetY = containerHeight - topPos - 36;
            }
        }
